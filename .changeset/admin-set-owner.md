...
- New optional env var `CGS_ADMIN_PASSWORD` (min 16 non-whitespace chars). When set, it enables the operator-only `app.certified.group.admin.*` endpoints, authenticated via HTTP Basic auth as user `admin` with this password. When unset, **all admin endpoints are disabled** and every request to them returns `401` — there is no insecure default.
