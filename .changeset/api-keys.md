...
- **Use a key:** send `X-API-Key: cgsk_…` instead of `Authorization: Bearer <jwt>`, and name the group with `repo` (querystring for queries, body for procedures) — the same request-level targeting the JWT path uses. No nonce, no 2-minute lifetime; the key is valid until revoked. Iteration 1 grants only `member.list`.
