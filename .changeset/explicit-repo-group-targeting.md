...
- **Mint tokens with `aud` = the service DID:** `getServiceAuth({ aud: <serviceDid>, lxm })` instead of `aud: <groupDid>`. Adding `repo` is what activates the new path — a request that carries `repo` is never treated as legacy, regardless of `aud`.
