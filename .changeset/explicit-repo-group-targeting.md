...
Apps now name the target group with an explicit `repo` field instead of overloading the login token's audience.
...
- **Migrating off the legacy path:** add `repo` (silences the deprecation signal) and mint `aud` = the service DID. Legacy responses carry RFC 8594 `Deprecation: true` + a `Link` header so you can spot un-migrated calls; no `Sunset` date is set yet.
