...
- **Both direct and proxied calls can migrate.** The user's PDS signs the service-auth token in both cases (via `getServiceAuth`); what differs is who sets `aud`. Direct: you pass `aud` = the service DID to `getServiceAuth` and present the returned token yourself. Proxied: the PDS sets `aud` from the proxy target, so target the service DID — `withProxy('certified_group_service', cgsServiceDid)`; the service now publishes a resolvable `did:web` document at `/.well-known/did.json` so the PDS can resolve it (#29). The legacy `withProxy('certified_group', groupDid)` stays on the deprecated `aud` = group DID form.
