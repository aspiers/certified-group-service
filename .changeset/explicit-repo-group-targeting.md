...
- **Direct calls migrate fully today; service proxying does not yet** — a proxying PDS mints `aud` from the proxy target (the group DID), so proxied calls stay on the legacy `aud` until the service publishes a resolvable `did:web` document (upcoming, #29). Use direct calls where full migration matters.
...
- Full migration steps, the service-DID resolution chain, and the security rationale: `docs/integration-guide.md` ("Migrating from the legacy `aud` form"), `docs/api-reference.md` ("Determining the service DID"), and `docs/design/aud-deprecation.md`.
