...
| `ADMIN_PASSWORD`        | No       | —                       | Enables the operator-only `app.certified.group.admin.*` endpoints (HTTP Basic auth, user `admin`); disabled when unset. Min 16 non-whitespace chars. See [deployment](docs/deployment.md#admin-endpoints). |
